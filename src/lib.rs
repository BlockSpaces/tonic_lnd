...
pub type LoopClient =
    looprpc::swap_client_client::SwapClientClient<InterceptedService<Channel, MacaroonInterceptor>>;
pub type FaradayServerClient = frdrpc::faraday_server_client::FaradayServerClient<
    InterceptedService<Channel, MacaroonInterceptor>,
>;
pub type InvoicesClient =
    invoicesrpc::invoices_client::InvoicesClient<InterceptedService<Channel, MacaroonInterceptor>>;
pub type WalletUnlockerClient = lnrpc::wallet_unlocker_client::WalletUnlockerClient<
    InterceptedService<Channel, MacaroonInterceptor>,
>;
pub type StateClient =
    lnrpc::state_client::StateClient<InterceptedService<Channel, MacaroonInterceptor>>;
...
impl tonic::service::Interceptor for MacaroonInterceptor {
    fn call(&mut self, mut request: tonic::Request<()>) -> Result<tonic::Request<()>, Error> {
        #[cfg(feature = "tracing")]
        {
            use tracing::debug;
            debug!(
                "MacaroonInterceptor called for method: {}",
                request.uri().path()
            );
            debug!("Macaroon hex length: {}", self.macaroon.len());
            debug!(
                "Macaroon hex preview: {}",
                &self.macaroon.chars().take(32).collect::<String>()
            );
        }
        let macaroon_bytes = match hex::decode(&self.macaroon) {
            Ok(bytes) => {
                #[cfg(feature = "tracing")]
                {
                    ...
                    debug!(
                        "Decoded macaroon bytes preview: {:?}",
...
async fn load_macaroon(
    ...
    #[cfg(feature = "tracing")]
    {
        tracing::debug!(
            "Final macaroon hex: {}",
            &hex_macaroon[..std::cmp::min(64, hex_macaroon.len())]
        );
        tracing::debug!("Final macaroon bytes length: {}", macaroon_bytes.len());
        tracing::debug!(
            "Final macaroon bytes preview: {:?}",
            &macaroon_bytes[..std::cmp::min(32, macaroon_bytes.len())]
        );
    }
    Ok(hex_macaroon)
}
    ...
    MP: AsRef<Path> + Into<PathBuf> + std::fmt::Debug,
{
    ...
    {
        info!(
            "Connecting to LND - Address: {:?}, Cert file: {:?}, Macaroon file: {:?}",
    ...
    let client = Client {
        ...
        loopclient: looprpc::swap_client_client::SwapClientClient::with_interceptor(
            conn.clone(),
            interceptor.clone(),
        ),
        faraday: frdrpc::faraday_server_client::FaradayServerClient::with_interceptor(
            conn.clone(),
            interceptor.clone(),
        ),
    ...
    <A as TryInto<tonic::transport::Endpoint>>::Error: std::error::Error + Send + Sync + 'static,
{
    ...
    {
        info!(
            "Connecting to LND with in-memory certificates - Address: {:?}",
            address
        );
        debug!(
            "Certificate hex length: {}, Macaroon hex length: {}",
            cert_file_as_hex.len(),
            macaroon_as_hex.len()
        );
    }
...
mod tls {
    ...
    pub(crate) async fn config_with_hex(
        file_as_hex: String,
    ) -> Result<tonic::transport::ClientTlsConfig, ConnectError> {
        #[cfg(feature = "tracing")]
        {
        ...
        if skip_verify {
            ...
            tls_config
                .dangerous()
                .set_certificate_verifier(std::sync::Arc::new(AcceptAnyCertVerifier));
        } else {
    ...
    impl CertVerifier {
        ...
        pub(crate) async fn load_as_hex(file_as_hex: String) -> Result<Self, InternalConnectError> {
            ...
            {
                info!(
                    "Loading certificates from hex string of length {}",
                    file_as_hex.len()
                );
            }
